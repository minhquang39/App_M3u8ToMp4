...
- Remembers recent URL, output location, preferred tool paths, engine choice, and N_m3u8DL-RE selection presets between sessions.
...
3. Run the app:
   ...
   - For `N_m3u8DL-RE`, optionally tweak thread count and track selection (video/audio/subtitle) before starting.
...
- Progress estimates differ by engine: FFmpeg shows timecodes when available, while `N_m3u8DL-RE` surfaces its console log lines and track selection output. The app now supplies `--select-video`/`--select-audio` automatically (default `best`) so no interactive prompts appear.
